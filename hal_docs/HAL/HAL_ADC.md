...
| 场景 | 推荐方案 |
|------|----------|
| 偶尔读一次传感器值（按键触发或主循环轮询） | 单次阻塞：`Start` + `PollForConversion` + `GetValue` |
| 主循环持续读取单路模拟量 | 连续模式 `ContinuousConvMode=ENABLE`，轮询读取 |
| 多路传感器（≥2路）同时采集 | 扫描模式 + DMA，`HAL_ADC_Start_DMA`，结果自动填入数组 |
| 高速采集，不想占用 CPU 搬运数据 | DMA 循环模式 + `HAL_ADC_ConvCpltCallback` 处理 |
| 读取片内温度 | `ADC_CHANNEL_TEMPSENSOR`，采样时间必须用 239.5 周期 |
| 每隔固定时间精确采样一次 | 定时器 TRGO 触发，配置 `ExternalTrigConv` 触发源 |

**采样时间怎么选**

| 信号类型 | 推荐采样时间 |
|----------|-------------|
| 低阻抗信号（分压电阻 <1kΩ） | 1.5 / 7.5 周期 |
| 普通传感器（阻抗 <10kΩ，如电位器） | 28.5 / 55.5 周期 |
| 高阻抗信号、长走线、信号源内阻大 | 71.5 / 239.5 周期 |
| 片内温度传感器 | 必须 239.5 周期 |

> 采样时间越长越精确，但转换速率越低。ADC 时钟推荐 12MHz（PCLK2=72MHz，6 分频）。首次使用前必须调用 `HAL_ADCEx_Calibration_Start`。
