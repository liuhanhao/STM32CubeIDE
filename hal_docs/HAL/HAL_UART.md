...
| 场景 | 推荐模式 | 函数 |
|------|----------|------|
| 调试打印、偶发少量数据（<几十字节） | 阻塞 | `HAL_UART_Transmit` / `Receive` |
| CPU 需同时处理其他事，交互式命令 | 中断 | `HAL_UART_Transmit_IT` / `Receive_IT` |
| 大量数据传输（日志、图像、固件升级） | DMA | `HAL_UART_Transmit_DMA` / `Receive_DMA` |
| 不定长数据接收（不知道会来多少字节） | 中断逐字节 或 空闲中断 | 每次 `Receive_IT` 接 1 字节；或 `HAL_UARTEx_ReceiveToIdle_IT` |

**常见项目场景**

| 项目 | 配置建议 |
|------|----------|
| 调试串口 / printf 重定向 | USART1，115200，阻塞发送，`fputc` 重定向 |
| 与上位机通信 | 中断接收逐帧解析，DMA 发送大数据帧 |
| GPS / 蓝牙模块 | DMA 循环接收 + 空闲中断检测帧尾 |
| MODBUS RTU | 严格时序，DMA 接收，空闲中断判断帧结束 |
| 多串口系统 | 低频串口用中断，高频/大数据串口用 DMA |

> 阻塞接收（`HAL_UART_Receive`）会卡住 CPU 等待，生产代码中尽量避免。
