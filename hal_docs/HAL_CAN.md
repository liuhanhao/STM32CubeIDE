...
| 场景 | 说明 |
|------|------|
| 多板嵌入式系统（机器人各关节板、传感器板通信） | 一根总线连所有节点，减少布线 |
| 工业控制（变频器、PLC、伺服驱动器通信） | 抗干扰能力强，适合强电气噪声环境 |
| 汽车电子（OBD 诊断、车身控制模块） | CAN 是汽车标准通信协议 |
| 替代多路 UART 节省引脚 | 多板通信只需 CANH/CANL 两根线 |

**CAN vs UART/RS485 对比**

| | CAN | UART / RS485 |
|--|-----|-------------|
| 多节点支持 | 天然支持，最多 ~110 节点 | 需要应用层地址协议（如 MODBUS） |
| 总线仲裁 | 硬件自动仲裁，低 ID 优先，不会冲突 | 需软件协调发送时机 |
| 错误检测 | 内置 CRC + 位填充，自动重发 | 仅奇偶校验，无自动重发 |
| 最高速率 | 1Mbps | RS485 约 10Mbps |

**过滤器配置建议**

| 场景 | 配置 |
|------|------|
| 开发调试，接收所有消息 | `FilterMaskIdHigh/Low = 0x0000`（掩码全 0） |
| 生产环境，只接收特定节点 | 配置具体 ID + 掩码，减少不必要的 CPU 中断 |

**波特率计算参考（APB1 = 36MHz）**

| 目标波特率 | Prescaler | TimeSeg1 | TimeSeg2 |
|-----------|-----------|----------|----------|
| 1Mbps | 4 | 7TQ | 2TQ |
| 500Kbps | 9 | 7TQ | 2TQ |
| 250Kbps | 9 | 7TQ | 8TQ |
...
> CAN 总线两端各需 120Ω 终端电阻，否则信号反射导致通信异常。
